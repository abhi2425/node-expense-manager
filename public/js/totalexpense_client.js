console.log("Hii from client!!")

fetch('/totalexpense').then((response) => {
    response.json().then((result) => {
        ...
        result.forEach(element => {
            ...
            max_expense.innerHTML = "Max Expense-:" + expense[expense.length - 1] + "$"

            const min_expense = document.querySelector(".min_expense")
            min_expense.innerHTML = "Min Expense-:" + expense[0] + "$"

            const avg_expense = document.querySelector(".avg_expense")
            avg_expense.innerHTML = "Average Expense-:" + avgSum + "$"
